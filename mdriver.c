...
int main(int argc, char **argv)
{
    ...
    /*
     * Check if some team information is missing
     */
    if (team_check) {
        ...
        /*
         * If no -f command line arg, then read from stdin
         */
        if (tracefiles == NULL) {
            num_tracefiles = 0;
        }
    ...
    /* Evaluate libc malloc on all traces */
    if (run_libc) {
        printf("Skipping libc evaluation because we read from stdin.\n");
    }

    /* Evaluate mm malloc on all traces */
    if (verbose > 1)
        printf("\nTesting mm malloc\n");

    int actual_num_traces = (num_tracefiles > 0) ? num_tracefiles : 1;

    /* Allocate mm stats array, with one stats_t struct per tracefile */
    mm_stats = (stats_t *)calloc(actual_num_traces, sizeof(stats_t));
    if (mm_stats == NULL)
    ...
    for (i=0; i < actual_num_traces; i++) {
        char *filename = (num_tracefiles > 0) ? tracefiles[i] : "stdin";
        trace = read_trace(tracedir, filename);
    ...
    if (verbose) {
        ...
        printresults(actual_num_traces, mm_stats);
        printf("\n");
    }
    ...
    avg_mm_util = util/actual_num_traces;
    ...
    if (errors == 0) {
        ...
        printf("Perf index = %.0f (util) + %.0f (thru) = %.0f\n",
               p1*100,
               p2*100,
               perfindex);
...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    if (strcmp(filename, "stdin") == 0) {
        ...
        if ((tracefile = fopen(path, "r")) == NULL) {
            /* Try opening directly if tracedir + filename fails */
    ...
    while (fscanf(tracefile, "%s", type) != EOF) {
        ...
        case 'f':
            ...
            break;
        default:
            printf("Ignoring invalid op type: %s\n", type);
            continue;
        }
        op_index++;
    }
    if (tracefile != stdin) fclose(tracefile);
